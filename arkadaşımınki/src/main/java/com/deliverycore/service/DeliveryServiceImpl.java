...
public class DeliveryServiceImpl implements DeliveryService {
    ...
    public void setV11Services(SeasonService seasonService) {
    ...
    public Optional<ActiveEvent> startEvent(String deliveryName, boolean force) {
        ...
        if (defOpt.isEmpty()) { LOGGER.warning("Delivery not found: " + deliveryName); return Optional.empty(); }
        
        DeliveryDefinition def = defOpt.get();
        if (!force && !def.enabled()) { LOGGER.info("Delivery is disabled: " + deliveryName); return Optional.empty(); }
        
        if (def.season().enabled() && seasonService != null) {
            if (!seasonService.isSeasonActive(def.season())) { LOGGER.info("Delivery is not in season: " + deliveryName); return Optional.empty(); }
        }
        
        try {
            Category category = categoryService.resolveCategory(def.category().mode(), def.category().value());
            if (category == null || category.items().isEmpty()) { LOGGER.warning("Category is empty for delivery: " + deliveryName); return Optional.empty(); }
            
            String item = categoryService.resolveItem(category, def.item().mode(), def.item().value());
            if (item == null || item.isEmpty()) { LOGGER.warning("Item could not be resolved for delivery: " + deliveryName); return Optional.empty(); }
    ...
    public List<Winner> endEvent(String deliveryName) {
        ...
        if (schedulerService != null) schedulerService.cancelScheduledEvent(deliveryName);
        
        int winnerCount = event.getWinnerCount();
        if (winnerCount <= 0) winnerCount = deliveryConfig.getDelivery(deliveryName).map(DeliveryDefinition::winnerCount).orElse(1);
    ...
    @Override
    public Optional<ActiveEvent> getActiveEvent(String deliveryName) { return Optional.ofNullable(activeEvents.get(deliveryName)); }
    
    @Override
    public List<ActiveEvent> getAllActiveEvents() { return List.copyOf(activeEvents.values()); }
