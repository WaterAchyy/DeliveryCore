...
public class WebhookHandler {
    ...
    public void sendStartWebhook(String delivery, String item, String category) {
        if (!isValid()) {
            logger.info("[WEBHOOK] Gonderilmedi - enabled: " + enabled + ", url: " + (webhookUrl != null ? webhookUrl.length() : 0) + " chars");
            return;
        }
    ...
    public void sendTestWebhook() {
        ...
        logger.info("[WEBHOOK TEST] Enabled: " + enabled);
        logger.info("[WEBHOOK TEST] URL: " + (webhookUrl != null ? webhookUrl.substring(0, Math.min(50, webhookUrl.length())) + "..." : "null"));
